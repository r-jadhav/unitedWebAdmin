...
const AddPoModal = ({projectId,supplierId,addPurchaseOrderModal,setAddPurchaseOrderModal,PurchaseOrderId}) => {
    ...
    const insertProduct = () => {
        api.post('/purchaseorder/insertPurchaseProduct',productDetail)
        .then((res) => {
    ...
    <>
         <Modal size="xl" isOpen={addPurchaseOrderModal}>
            ...
            <ModalBody>
                <FormGroup>
                    ...
                    <Col md="12" className='mb-4'>
                        ...
                        <Col md="3">
                            <Button color="primary" className='shadow-none' onClick={()=>{AddNewLineItem()}}>Add Item</Button>
                        </Col>
                        </Row>
                        <br/>
                        {/* <Row>
                    ...
                    {addMoreItem.map((item) =>
                     {
                      ...
                      <Input value={item.title} type="hidden" name="title"></Input>
                            ...
                            <td data-label="Cost Price" ><Input defaultValue={item.cost_price} type="number" value={insertPurchaseOrderData&& insertPurchaseOrderData.cost_price} 
                                  name="cost_price" /></td>
                            <td data-label="Selling Price"><Input type="input" defaultValue={item.price} name="mrp" value={insertPurchaseOrderData&& insertPurchaseOrderData.mrp} />{item.price}</td>
                            <td data-label="GST"><Input type="number" defaultValue={item.gst} name="gst" value={insertPurchaseOrderData&& insertPurchaseOrderData.gst} /></td>
            ...
            <ModalFooter>
                ...
                <Button color="secondary" className='shadow-none' onClick={()=>{setAddPurchaseOrderModal(false)}}>Cancel</Button>
            </ModalFooter>
        </Modal>
